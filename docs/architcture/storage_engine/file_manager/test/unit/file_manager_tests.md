...
└── tests/
    ...
    │   ├── test_file_io.py                   # Testing Reader/Writer
    │   └── test_file_synchronizer.py         # Testing Sync & Expansion
    │
...
| UT-SYNC-04 | Allocate Physical File | OS creates physical namespace | call `allocate_on_disk(path)` | Intercepts `os.open` verifying creation intent reaches physical partition |
